...
export default function CalenderComponent() {
  ...
  return (
    <div className="p-5 bg-white dark:bg-[#1D1E24]">
      <h3 className="text-lg font-semibold mb-5">Select Date</h3>
      <GenarelDropdownComponent dropdownInfo={dropdownInfo}>
        <div className="flex items-center justify-between font-semibold text-dark2">
          June 2025 <ChevronDown />
        </div>
      </GenarelDropdownComponent>

      <div className="grid grid-cols-7 p-6.5 bg-[#F7FAFC] rounded-2xl gap-5 w-full mt-5">
        ...
        {Array.from({ length: 30 }).map((_, index) => (
          <div className="font-semibold rounded-full">{index + 1}</div>
        ))}
      </div>
    </div>
  );
